...
import 'package:tutornest/pages/Student/become_a_tutor.dart';
        ...
        children: [
          ...
          ListTile(
            ...
            trailing: const Icon(Icons.arrow_forward),
              onTap: () async {
                try {
                  // Await the result of reading from FlutterSecureStorage
                  final studentid = await FlutterSecureStorage().read(key: 'userId');

                  // Query the payments collection
                  final checkPayment = await FirebaseFirestore.instance
                      .collection('payments')
                      .where('studentId', isEqualTo: studentid)
                      .get();

                  // Navigate based on whether payment documents exist
                  if (checkPayment.docs.isEmpty) {
                    Navigator.push(
                      context,
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => ActiveSubscriptionPage()),
                    );
                  }
                } catch (e) {
